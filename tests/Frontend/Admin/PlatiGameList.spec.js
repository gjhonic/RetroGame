...
import PlatiGameList from '../../../assets/vue/Admin/PlatiGameList.vue';
...
function onePageResponse(overrides = {}) {
    return { items: [samplePlatiGame], total: 1, page: 1, totalPages: 1, ...overrides };
}

async function mountList(response = onePageResponse()) {
    mockFetchOnce(response);
    const wrapper = mount(PlatiGameList);
    await flushPromises();
...
describe('PlatiGameList — загрузка', () => {
    ...
    it('показывает состояние ошибки при неудачном запросе', async () => {
        ...
        expect(wrapper.text()).toContain('Не удалось загрузить Plati-игры');
    });
    ...
    it('без связанной игры показывает прочерк вместо ссылки', async () => {
        const wrapper = await mountList(onePageResponse({
            items: [{ ...samplePlatiGame, gameId: null, gameName: null }],
        }));
...
describe('PlatiGameList — постраничная навигация', () => {
