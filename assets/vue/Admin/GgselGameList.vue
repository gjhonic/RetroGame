<template>
    <div v-if="error" class="alert alert-danger">
        Не удалось загрузить Ggsel-игры: {{ error }}
    </div>

    <template v-else>
        <div v-if="loading" class="d-flex align-items-center gap-2 text-muted py-5">
            <div class="spinner-border spinner-border-sm" role="status"></div>
            <span>Загружаем Ggsel-игры…</span>
        </div>

        <div v-else-if="ggselGames.length === 0" class="alert alert-secondary">
            Ничего не найдено.
        </div>
                    ...
                    <tbody>
                        <tr v-for="row in table.getRowModel().rows" :key="row.id">
                            <td v-for="cell in row.getVisibleCells()" :key="cell.id">
                                <template v-if="cell.column.id === 'gameName'">
                                    <a
                                ...
                                <a
                                    ...
                                    :href="`/admin/ggsel-games/${row.original.id}`"
...
// Ключ колонки на клиенте -> имя поля sortBy на бэкенде (только там, где отличается).
...
const ggselGames = ref([]);
const total = ref(0);
const totalPages = ref(1);
const loading = ref(true);
const error = ref(null);
const filters = reactive({ gameName: '', url: '' });
const sorting = ref([]);
...
async function loadGgselGames() {
    ...
    if (sort) {
        params.set('sortBy', sortParamNames[sort.id] ?? sort.id);
        params.set('sortDir', sort.desc ? 'desc' : 'asc');
    }

    try {
        const response = await fetch(`/api/admin/ggsel-games?${params}`);
