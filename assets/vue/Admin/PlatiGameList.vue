<template>
    <div v-if="error" class="alert alert-danger">
        Не удалось загрузить Plati-игры: {{ error }}
    </div>

    <template v-else>
        <div v-if="loading" class="d-flex align-items-center gap-2 text-muted py-5">
            <div class="spinner-border spinner-border-sm" role="status"></div>
            <span>Загружаем Plati-игры…</span>
        </div>

        <div v-else-if="platiGames.length === 0" class="alert alert-secondary">
            Ничего не найдено.
        </div>
                        ...
                        <tr v-for="row in table.getRowModel().rows" :key="row.id">
                            <td v-for="cell in row.getVisibleCells()" :key="cell.id">
                                ...
                                <a
                                    ...
                                    :href="`/admin/plati-games/${row.original.id}`"
...
async function loadPlatiGames() {
    ...
    try {
        const response = await fetch(`/api/admin/plati-games?${params}`);
