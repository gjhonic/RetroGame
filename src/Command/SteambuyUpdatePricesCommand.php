...
/**
 * Команда обновления цен на игры Steambuy.
 *
 * Системное имя: app:steambuy-update-prices
 *
 * Назначение:
 *   Получает актуальные цены на игры из Steambuy и сохраняет их в историю цен.
 *
 * Логика работы:
 *   - Находит все игры Steambuy, для которых требуется обновление цены (shop=Steambuy, shouldImportPrice=true).
 *   - Для каждой игры:
 *       - Проверяет, обновлялась ли цена сегодня.
 *       - Получает страницу игры с сайта Steambuy.
...
class SteambuyUpdatePricesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $logsCron->setCronName('steambuy-update-prices');
            ...
            try {
                ...
                if (
                    ...
                    foreach ($matches as $match) {
                        ...
                        if ('наличие' === mb_strtolower($label)) {
                ...
                if (preg_match('/<div class=\"product-price__cost\">\s*(.*?)\s*<\/div>/', $html, $matches)) {
                    ...
                    } elseif ('скоро' === mb_strtolower($priceText)) {
