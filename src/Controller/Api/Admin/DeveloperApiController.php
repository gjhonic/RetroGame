...
/** JSON API справочника разработчиков для админки — используется Vue-компонентом Admin/NamedEntityList. */
#[Route('/api/admin/developers')]
#[IsGranted('ROLE_MODERATOR')]
#[OA\Tag(name: 'Admin/Developers')]
class DeveloperApiController extends AbstractController
{
    ...
    /**
     * Страница списка разработчиков для таблицы в админке: фильтр по названию,
     * сортировка и постраничная навигация выполняются в БД.
     */
    #[Route('', name: 'app_api_admin_developer_list', methods: ['GET'])]
    #[OA\Parameter(
        name: 'page',
    ...
    #[OA\Response(
        response: 200,
        description: 'Страница списка разработчиков с постраничной навигацией',
    ...
    public function list(Request $request, DeveloperRepository $developerRepository): JsonResponse
    {
        ...
        $total = $developerRepository->countForAdminList($filters);
        $totalPages = max(1, (int) ceil($total / $perPage));
        $page = min(max(1, $request->query->getInt('page', 1)), $totalPages);

        $items = $developerRepository->findForAdminList(
            $filters,
            $sortField,
            $sortDir,
            $perPage,
