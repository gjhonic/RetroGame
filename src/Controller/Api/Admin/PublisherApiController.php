...
/** JSON API справочника издателей для админки — используется Vue-компонентом Admin/NamedEntityList. */
#[Route('/api/admin/publishers')]
#[IsGranted('ROLE_MODERATOR')]
#[OA\Tag(name: 'Admin/Publishers')]
class PublisherApiController extends AbstractController
{
    ...
    /**
     * Страница списка издателей для таблицы в админке: фильтр по названию,
     * сортировка и постраничная навигация выполняются в БД.
     */
    #[Route('', name: 'app_api_admin_publisher_list', methods: ['GET'])]
    #[OA\Parameter(
        name: 'page',
    ...
    #[OA\Response(
        response: 200,
        description: 'Страница списка издателей с постраничной навигацией',
    ...
    public function list(Request $request, PublisherRepository $publisherRepository): JsonResponse
    {
        ...
        $total = $publisherRepository->countForAdminList($filters);
        $totalPages = max(1, (int) ceil($total / $perPage));
        $page = min(max(1, $request->query->getInt('page', 1)), $totalPages);

        $items = $publisherRepository->findForAdminList(
            $filters,
            $sortField,
            $sortDir,
            $perPage,
